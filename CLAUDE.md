...
27. **Anything whose answer depends on WHERE the code runs is a dual-transport hazard — and rules 19–26 do not cover it.** Those rules police the *shape* of what crosses the wire. This one polices the *ambient environment on the far side of it*. On desktop, `packages/core` executes on the shop's own machine in Beirut. On web, the identical code executes on a Fly container in Frankfurt with no `TZ` set, i.e. UTC. Every function that reads its answer from the machine rather than from its arguments therefore returns something different on the two transports while the code reads as obviously correct.

    **The hazards, concretely:** the clock and calendar day (`new Date()`, `Date.now()`, `localDay()`, SQLite's `date('now')`), timezone, locale and number/date formatting (`toLocaleString`), the filesystem, `process.env`, and anything derived from the host. **Detection:** grep `packages/core/src` for those on a REQUEST path (migrations don't count) and ask of each — *could the client have told us this instead?* If yes, it is a bug waiting for the right hour of the day.
