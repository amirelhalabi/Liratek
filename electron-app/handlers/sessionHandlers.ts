...
export function registerSessionHandlers() {
  ...
  // Update session
  ipcMain.handle('session:update', async (_event, sessionId: number, data: {
    customer_name?: string;
    customer_phone?: string;
    customer_notes?: string;
  }) => {
  ...
  // Close session
  ipcMain.handle('session:close', async (_event, sessionId: number, closedBy: string) => {
  ...
  ipcMain.handle('session:linkTransaction', async (_event, data: {
