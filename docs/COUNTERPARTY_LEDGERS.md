...
'ADJUSTMENT','SETTLEMENT','CASH_PRIZE','SUPPLIER_PAYS_US','DISCOUNT','STOCK_INTAKE')), amount_usd,
...
every row (same 12-step pattern as v83/v98/v99/v127). **v164**
(`SUPPLIER_STOCK_INTAKE_PLAN.md`) widened it again the same way to add
`'STOCK_INTAKE'` — receiving stock with a supplier attached.
...
| `DISCOUNT`         | `SupplierRepository._postSupplierDiscount` (CQ-10 — bundled with a PAY-direction cashflow only; the standalone write-off caller was **REMOVED** by `SUPPLIER_STOCK_INTAKE_PLAN.md` owner decision D8 — `SupplierService.writeOffSupplierDebt` / `SupplierRepository.writeOffSupplierDebt` no longer exist at any layer; RECEIVE-direction rejects a bundled discount at the data layer — a supplier can't simultaneously pay the shop and forgive what the shop owes them, see §9)                                                                                                                                                                                                              | The wrapping transaction is `COUNTERPARTY_DISCOUNT`, **`NON_REVERSIBLE`**; correction is an opposite manual discount. Runs the same FIFO purchase-coverage pass a real `PAY` cashflow does (`allocateFifo`, §9) so open `supplier_purchases` rows don't stay stuck deferred                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `STOCK_INTAKE`     | **Automatic only**: `SupplierRepository.recordStockIntake`, called from `InventoryService.receiveStock` (Add Product / restock forms) whenever a supplier is attached AND the per-entry "old stock" checkbox is unticked (D6). Writes `+qty × unit_cost_usd` (shop owes more — same sign as `TOP_UP`), the sale-side FIFO cost batch (`StockBatchRepository.createBatch`), and its own `SUPPLIER_STOCK_INTAKE` transaction, all inside ONE `db.transaction()`. Stock quantity changes, sales, refunds, deletes and cost-price edits never write or move this row — that recompute-from-live-inventory formula is exactly the bug v164 replaced (`getProductSupplierBalances`, FEATURE_GUIDE §8) | The wrapping transaction (`SUPPLIER_STOCK_INTAKE`) is **REVERSIBLE**: `TransactionRepository._reverseSupplierStockIntake` is the rule-20 owner. The generic void path soft-voids this `supplier_ledger` row for free (`source_table = 'supplier_ledger'`); this method additionally deletes the linked stock batch (`StockBatchRepository.deleteBatchForVoid`) and lowers `products.stock_quantity` by the batch's original `quantity`. It **REFUSES the whole void** (throws, rolling back the reversal transaction the generic path already wrote) when any unit from that batch was already sold — a batch a sale drew cost from cannot be silently erased. No payments row / drawer leg exists to reverse (this type funds neither) |
