...
| #   | Task                                                | Status                                              | Details                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --- | --------------------------------------------------- | --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T2  | **Rate change on Account page mutates LBP balance** | ✅ **Done**                                         | Fixed 2026-07-12, full rollout complete (MCP-0…5). All payment forms now feed per-currency `totals` into MultiPaymentInput, which runs on the money engine (`packages/ui/src/money/`) — a rate is only consulted when a payment crosses currencies, so an LBP debt paid in LBP is rate-invariant. Guards: component tests + `lira-105` e2e (proven failing-first: 600,000 → 606,742 pre-fix). EUR-ready: a new currency = rate-table/registry row, zero component changes. Plan + full record: `docs/plans/done_plans/MULTI_CURRENCY_PAYMENT_PLAN.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| T3  | **Allow zero return in "Return amount" section**    | ✅ **Done — live everywhere**                       | Shipped 2026-07-13 (KC-0…4): "Keep change" button on POS, debt repayments, custom services, maintenance, loto, telecom recharges, OMT/Whish app transfers, OMT/Whish/Katch/crypto catalog forms (kept rides the legs-carrying txn), and session-basket checkout (standalone non-voidable `KEPT_CHANGE` profit row, owner decision). Kept extra books as per-currency profit (tender-native), refund-symmetric, drawer/closing exact; debts/sessions kept change shows on the new "Other / Kept Change" Profits line. Guards: lira-106/107/108/109 (all failing-first). Full record: `docs/plans/done_plans/T3_KEEP_CHANGE_PLAN.md`.                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| T5  | **Receipt logo + editable walk-in customer (POS)**  | ✅ **Done**                                         | Done 2026-07-13 (RCP-0 6e694b4, RCP-1 8dd83df): (a) logo uploaded in Settings (base64), shown atop every receipt via a shared print path; (b) walk-in customer editable — typed at checkout AND renamable on a completed sale (Sale Detail → edit, gated to anonymous sales, persisted to the unified transaction rule 11). Guards: printReceipt unit test + lira-111. Plan: docs/plans/done_plans/RECEIPTS_PLAN.md.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| T6  | **Loto: cashed-out prizes reduce supplier balance** | 🟡 **Verify on shop machine — fix already shipped** | Investigated 2026-07-13: the bug was an INVERTED loto ledger sign (prizes made the balance BIGGER). Fixed in **v1.29.0 (2026-07-05)** incl. historical repair (migration v119); both prize flows (Loto page + session basket) write the ledger; guarded by `lira-091-loto-ledger-sign` e2e reading the same `getBalances` API the Suppliers page shows. **Action:** confirm shop app ≥ v1.29.0, then check balance = sales − commission − prizes. If STILL wrong on v1.29.x → old rows with non-standard note text escaped the repair; needs a copy of the shop DB + a targeted data-repair migration. **Found while testing (2026-07-13, fixed, uncommitted):** recording a prize WITHOUT a ticket number failed "Validation failed: ticket_number: Required" — `lotoCashPrizeSchema` + `LotoService.recordCashPrize` wrongly required it (optional everywhere else; the session-basket replay hit the same wall at checkout). Fixed in the shared core validator (covers IPC + REST), proven failing-first via lira-091's new no-ticket prize case. |
| T7  | **Expenses: enable all payment methods**            | ✅ **Done**                                         | Done 2026-07-13: Expenses form now offers every drawer-affecting DB method (Cash, OMT Wallet, Whish Wallet, Binance) — dropped the emoji labels and the phantom "Credit Card" (never a real payment_methods row). An expense = shop paying OUT, so the method picks which drawer drains; CUSTOMER_ACCOUNT/GIFT_CARD stay excluded (affects_drawer=0, no customer). Binance is USDT-denominated: dollar value stored in amount_usd, drawer leg deducts USDT (1:1), void restores USDT via the generic reversal's currency_code — no migration (owner confirmed they pay expenses in USDT). Guard: `lira-110` (failing-first: Binance USDT delta 0 pre-fix). Owner chose to drop Credit Card.                                                                                                                                                                                                                                                                                                                                                           |
| T8  | **Receipts for mobile services, maintenance, etc.** | ✅ **Done**                                         | Done 2026-07-13 (RCP-2 91a763e builder; RCP-3): detailed service receipts (amount + fee + payment split + change, never cost/price/profit) for all 5 service types — mobile services, recharge, maintenance, custom services, loto — reprintable via a "Print" action in the Transactions viewer (with the shop logo). Card-grid items show category/subcategory from the note, title-cased. One receipt per transaction. Guards: serviceReceipt unit tests + lira-112 (customer-leg filter). Optional polish (deferred): inline print-after-success button per module. Plan: docs/plans/done_plans/RECEIPTS_PLAN.md.                                                                                                                                                                                                                                                                                                                                                                                                                                 |
