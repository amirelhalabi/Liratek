...
| T5  | **Receipt logo + editable walk-in customer (POS)**   | ✅ **Done** | Done 2026-07-13 (RCP-0 6e694b4, RCP-1 8dd83df): (a) logo uploaded in Settings (base64), shown atop every receipt via a shared print path; (b) walk-in customer editable — typed at checkout AND renamable on a completed sale (Sale Detail → edit, gated to anonymous sales, persisted to the unified transaction rule 11). Guards: printReceipt unit test + lira-111. Plan: docs/plans/RECEIPTS_PLAN.md. |
