...
| T3  | **Allow zero return in "Return amount" section**    | 🟡 **Planned** | Confirmed 2026-07-13: the two CASH return fields auto-balance each other — returning nothing is structurally impossible. Owner decisions: everywhere (incl. debts), kept extra COUNTS AS PROFIT (per-currency stamp at create → refund symmetry automatic), "Keep change" toggle button. Plan + tickets KC-0…4: `docs/plans/T3_KEEP_CHANGE_PLAN.md`. |
