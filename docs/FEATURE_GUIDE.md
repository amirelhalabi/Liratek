...
| PCD cannot go negative — RECEIVE payout guard | RECEIVE payouts are **blocked**, per currency, before any payout leg posts, when the PCD lacks funds — `InsufficientDrawerFundsError` (`code: "INSUFFICIENT_DRAWER_FUNDS"`, `details.{shortfall,available,required}` per currency). The Services page catches `code` (never a message-string match) and offers an inline "move remaining from General" transfer with a USD/LBP toggle; the transaction proceeds once the transfer covers the shortfall. **Do not** reintroduce "it MAY go negative, by design" overdraft language near this drawer — that was the float model's semantics, deleted along with its credit/debit legs. No other leg (SEND cash-in, fees, change) can drive the PCD negative. | New UI-driven spec (lira-131 pattern, handover §4.1) — WILL own this; see `PRIMARY_CASH_DRAWER_PLAN.md` §4 |
