...
| **Product suppliers — event-based (v164)**                               | `getProductSupplierBalances` **used to** recompute "owed" on every read as `SUM(live stock × live cost) + SUM(ledger)` — a POS sale silently lowered the debt, a refund raised it, a cost edit re-priced already-settled history, and no intake path ever wrote a ledger row at all (`SupplierRepository.ts:1018-1052`, pre-v164). **SUPPLIER_STOCK_INTAKE_PLAN.md (owner decision D1)** replaced this with the same **ledger-sum-only** model company suppliers already use: `getProductSupplierBalances` now shares the SAME extracted ledger-balance expression as `getSupplierBalances`, restricted to non-system suppliers with a linked `product_suppliers` row. `entry_type` **`STOCK_INTAKE`** (migration v164) is written ONCE per delivery by `SupplierRepository.recordStockIntake`: **+qty × unit cost**, same sign convention as `TOP_UP` (shop owes more). Stock-quantity changes, sales, refunds, deletes, and cost-price edits **never** touch this ledger row — only a new intake (or a restock) does. An **"old stock"** per-entry checkbox on the Add Product / restock forms creates a FIFO cost batch (`StockBatchRepository`) but deliberately **skips** the ledger write (D6) — the batch still exists for profit costing, there is simply no debt to book. Reversal owner (rule 20): voiding the `SUPPLIER_STOCK_INTAKE` transaction is `TransactionRepository._reverseSupplierStockIntake` — the generic void path soft-voids the `supplier_ledger` row for free (`source_table = 'supplier_ledger'`), and this method additionally deletes the linked stock batch and lowers `products.stock_quantity` by the batch's original quantity; it **REFUSES** the whole void if any unit from that batch was already sold. Standalone supplier write-off was **REMOVED** (D8) — the bundled Pay-form discount (`DISCOUNT` entry type, above) is the only forgiveness path left. | `docs/plans/todo_plans/SUPPLIER_STOCK_INTAKE_PLAN.md`, `docs/COUNTERPARTY_LEDGERS.md` §4                                                                                       |
...
`supplier_ledger` tracks the **gross** `x + f` (SEND) / `−(x − f)` (RECEIVE) for `commission_model = 1` rows, and the pre-cutover `x + f − c` / `−(x − (f − c))`
...
> `Σ(drawer deltas) + Σ(receivable deltas) − Δ(owed to provider) = kept_change`
>
> **(LIRA-095 Phase 2: the right-hand side lost its `c`.)** Under the netted payable every transfer resolved to the shop’s own commission at CREATION time. With a gross payable it resolves to **zero** — nothing is earned at transaction time. The commission now appears once, at SETTLEMENT, as the real figure the operator enters, so `Σc` accrues to the PCD across the settlement cycle rather than per transfer.
...
| -------------------------------------------------------- | ---------- | -------------------------------------- | ------------- |
| SEND, fee on top (customer hands `x+f` cash)             | `+(x+f)`   | `+(x + f)`                             | `0`           |
| SEND, fee included (customer hands `x`; principal `x−f`) | `+x`       | `+((x−f) + f) = +x`                    | `0`           |
| RECEIVE, fee on top (payout `x`, fee `f` collected)      | `−x`, `+f` | `−(x − f)`                             | `0`           |
| RECEIVE, fee included (payout `x−f`)                     | `−(x−f)`   | `−(x − f)`                             | `0`           |
...
8. **Supplier/partner ledger** (§8): correct sign; amount = whatever quantity is actually
   ...
   **GROSS** `x + f` (SEND) / `−(x − f)` (RECEIVE) since LIRA-095 Phase 2 — the whole
   fee is owed and the commission settles separately (pre-cutover rows, `commission_model
= 0`, keep the old netted form); the principal `x` is real
