...
| Profits password gate (LIRA-177)                    | LIRA-177 (2026-09-07) | The `/profits` page and its 7 data endpoints moved from an admin ROLE gate to a per-page PASSWORD gate — both roles now reach the page (migration v163, `admin_only=0`) and everyone, admin included, must unlock. Shared `SetProfitsPasswordSchema`/`UnlockProfitsSchema` in `packages/core/src/validators/profits.ts`, re-exported by `electron-app/schemas/index.ts` with the zod-major cast and imported directly by backend; both transports call the same `ProfitsAccessService`. IPC `profits:{password-status,set-password,unlock,lock}` mirrors REST `GET /password-status`, `PUT /password` (admin only), `POST /unlock`, `POST /lock`. **Route-order gotcha:** those four MUST mount ABOVE `requireProfitsUnlock` in `backend/src/api/profits.ts` — they are what earns the unlock, so they can never sit behind it. Unlock state is per tenant+user, in-memory, 15-min TTL via core's `isProfitsUnlockLive`; **per-process**, so it needs shared storage before the backend ever runs multiple workers. Locked data routes answer **403** `{success:false,error:"Profits locked"}` — the one place here that does NOT follow the HTTP-200-on-failure envelope rule, matching the neighbouring `requireRole`. Adapter: 4 dual-mode fns in `backendApi.ts` → `ElectronApiAdapter` → `packages/ui/src/api/types.ts`. Proof: `backend/src/api/__tests__/profitsGate.api.test.ts` (14 tests incl. staff-403 on set-password, fail-closed, and TTL expiry driven through the clock). ⚠️ `frontend/tests/e2e-web/lira-web-029-profits-password-gate.spec.ts` is WRITTEN but the web suite has NOT been run against it — rule 19d is not yet satisfied. |
...
- **`ModuleContext` / `FeatureFlagContext` load before login, so the first
  render after signing in is half-empty.** Reported live 2026-09-08: right
  after logging in, the sidebar shows only 'Checkpoint Timeline', the
  dashboard is missing the Total Debt tile, and Drawer Balances shows one
  card instead of ten. A plain reload fixes it completely.

  Cause: `ModuleContext`'s effect calls `loadModules()` unconditionally on
  mount (`frontend/src/contexts/ModuleContext.tsx:50-57`) with no
  `isAuthenticated` gate, so on a cold login it fetches
  `/api/modules/enabled` before the JWT exists, gets rejected, caches an
  empty module list, and never re-fetches on the auth false -> true
  transition. A reload re-mounts it with the token already in localStorage,
  which is why the second render is correct. `FeatureFlagContext` has the
  same shape (zero `isAuthenticated` references).

  This is WEB-ONLY by construction, which is why it went unnoticed: over IPC
  the desktop renderer never needed a JWT, so an early fetch always
  succeeded. It only appears once the same provider talks to REST.

  Fix: apply the gate `CurrencyContext` and `MobileServiceItemsProvider`
  already use — take `isAuthenticated` from `useAuth()`, skip the load while
  false, and include it in the effect's deps so the false -> true transition
  triggers the fetch. `CurrencyContext.tsx:48-96` is the worked example, with
  the reasoning already written out there. Then add a regression test
  mirroring `CurrencyContext.authGate.test.tsx`.

  Worth checking for others at the same time: any provider under
  `frontend/src/contexts/` that fetches on mount and has zero
  `isAuthenticated` references is suspect. As of this note that is
  `ModuleContext` and `FeatureFlagContext`; `ActiveModuleContext` and
  `ThemeContext` hold local state only.

- Rotate the committed `DASHSCOPE_API_KEY` in `backend/.env.dev`; fix/remove corrupted `backend/.env.prod`.
- **DONE 2026-09-08 — self-service signup on the web.** `POST /api/auth/signup` (public, invite-code gated, rate-limited counting successes) feeds the same `provisionTenant()` as the super-admin route; page at `frontend/src/features/auth/pages/Signup.tsx`, route `/signup`, adapter fn `signup()` in `backendApi.ts`. Deliberately **not** `ipcOrHttp` — it is web-only by design, because Electron provisions its single tenant through the first-run wizard, and the login page hides the link when `isElectron()`. Operator switch and rationale: `docs/DEPLOYMENT.md` § 5b. Proof: `backend/src/api/__tests__/signup.api.test.ts` (12) + `frontend/src/features/auth/pages/__tests__/Signup.test.tsx` (11). No web e2e spec yet — signup mutates the tenant registry, so a spec needs a teardown story the web suite does not have.
...
- ~~**REST action routes don't WRITE audit entries.**~~ **This entry was WRONG — corrected 2026-09-08.** REST routes audit through `auditRest(...)` (`backend/src/middleware/audit.ts`), not the IPC-side `audit(...)`, and there are ~117 call sites. Both this entry and the validation-pass note below reached the same false conclusion from the same flawed evidence: `grep 'audit(' backend/src/api/*.ts` matches the IPC helper's name, which REST never uses, so it returns 0 and looks like proof. An audit-middleware "fix" was built on top of that and reverted (`0940de28`) once it was found to be writing DUPLICATE rows. The genuine gap was two route files with no audit call at all, now fixed; what remains is a few `servicePresets` PUT/DELETE routes. **If you re-audit this, grep `auditRest(`.**
