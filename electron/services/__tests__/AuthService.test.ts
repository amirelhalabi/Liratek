...
jest.mock('../../utils/crypto', () => ({
  ...
  validatePasswordComplexity: jest.fn().mockReturnValue({ valid: true, errors: [] }),
}));
...
describe('AuthService', () => {
  ...
  describe('login', () => {
    ...
    it('throws ValidationError for empty username', async () => {
      await expect(service.login('', 'password123')).rejects.toThrow(ValidationError);
    });

    it('throws ValidationError for empty password', async () => {
      await expect(service.login('testuser', '')).rejects.toThrow(ValidationError);
    });

    it('throws AuthenticationError for non-existent user', async () => {
      mockRepo.findByUsername.mockReturnValue(null);

      await expect(service.login('unknown', 'password123')).rejects.toThrow(AuthenticationError);
    });
    ...
    it('throws AuthenticationError for invalid password', async () => {
      ...
      await expect(service.login('testuser', 'wrongpassword')).rejects.toThrow(AuthenticationError);
    });
  ...
  describe('createUser', () => {
    ...
    it('throws ConflictError for duplicate username', async () => {
      (validatePasswordComplexity as jest.Mock).mockReturnValue({ valid: true, errors: [] });
      mockRepo.usernameExists.mockReturnValue(true);

      await expect(
        service.createUser(
          { username: 'existinguser', password: 'Password123!', role: 'cashier' },
          'admin'
        )
  ...
  describe('deactivateUser', () => {
    const mockUser = { id: 2, username: 'otheruser', role: 'cashier', is_active: 1 };
    ...
    it('throws AuthorizationError for non-admin actor', () => {
      expect(() => service.deactivateUser(2, 1, 'cashier')).toThrow(AuthorizationError);
    });

    it('throws BusinessRuleError when trying to deactivate self', () => {
      expect(() => service.deactivateUser(1, 1, 'admin')).toThrow(BusinessRuleError);
    });

    it('throws BusinessRuleError when deactivating last admin', () => {
      const adminUser = { id: 2, username: 'admin', role: 'admin', is_active: 1 };
      mockRepo.findById.mockReturnValue(adminUser as any);
      mockRepo.countActiveAdmins.mockReturnValue(1);

      expect(() => service.deactivateUser(2, 1, 'admin')).toThrow(BusinessRuleError);
    });
  });
  ...
  describe('reactivateUser', () => {
    ...
    it('throws AuthorizationError for non-admin actor', () => {
      expect(() => service.reactivateUser(2, 'cashier')).toThrow(AuthorizationError);
    });
  });
  ...
  describe('getAllUsersIncludingInactive', () => {
    ...
    it('throws AuthorizationError for non-admin actor', () => {
      expect(() => service.getAllUsersIncludingInactive('cashier')).toThrow(AuthorizationError);
    });
  });

  describe('getUserById', () => {
    it('returns user by ID', () => {
      const mockUser = { id: 1, username: 'testuser', role: 'cashier', is_active: 1 };
