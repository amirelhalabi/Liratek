...
export function registerOMTHandlers(): void {
    ...
    ipcMain.handle('omt:get-analytics', () => {
        try {
            ...
            `).get() as { today_commission_usd: number; today_commission_lbp: number; today_count: number };
            ...
            `).get() as { month_commission_usd: number; month_commission_lbp: number; month_count: number };
            ...
            const byProvider = db.prepare(`
                ...
                WHERE DATE(created_at) = DATE('now', 'localtime')
                GROUP BY provider
            `).all() as { provider: string; commission_usd: number; commission_lbp: number; count: number }[];
