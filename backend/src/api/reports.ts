...
router.post('/pdf', requireAuth, async (req, res) => {
  try {
    ...
    logger.info({ filename }, 'PDF generation requested (not available in backend mode)');
    res.json({ 
      success: false, 
...
router.post('/backup', requireAuth, async (_req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to backup database' });
  }
});
...
router.post('/restore', requireAuth, async (req, res) => {
  try {
    ...
    res.json({ success: false, error: 'Restore not yet implemented in backend' });
  } catch (error) {
...
router.get('/diagnostics/foreign-keys', requireAuth, async (_req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to check foreign keys' });
  }
});
...
router.get('/diagnostics/sync-errors', requireAuth, async (_req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to get sync errors' });
  }
});
