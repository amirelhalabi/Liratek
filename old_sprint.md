...
- [x] End-to-end verification in dev mode (`yarn dev`, perform FOR/THROUGH partner transactions manually)

#### Phase 7: Future Enhancements

- [x] Enrich ledger entries with transaction details (JOIN financial_services)
- [x] Enhanced ledger row display with mode badges (FOR = violet, THROUGH = sky)
- [x] Balance breakdown by mode (FOR / THROUGH / Settlements & Adj.) per currency
- [x] Filters by mode/provider/direction on Partners page
- [x] Transaction linking (ledger row → expandable panel with service details)
- [ ] Settlement improvements (mark individual entries as settled) — **follow-up**
...
- [x] Supplier ledger still records normally in all cases (we interact with provider regardless)
- [x] Existing "through partner" flow updated with explicit THROUGH mode
- [x] Normal (no partner) transactions unaffected
- [x] Partners page shows enriched transaction view with mode badges and details
- [x] Partners page filters by mode, provider, direction, date
- [x] Partners page balance breakdown by mode (FOR / THROUGH / other) per currency
- [x] Partner ledger entries link to original financial_services transaction (expandable row)
