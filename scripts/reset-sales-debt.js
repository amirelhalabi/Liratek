...
// Database path
const dbDir = path.join(process.env.HOME, 'Library', 'Application Support', 'liratek');
...
try {
    ...
    const saleItemsBefore = db.prepare('SELECT COUNT(*) as count FROM sale_items').get();
    const debtBefore = db.prepare('SELECT COUNT(*) as count FROM debt_ledger').get();
    ...
    try {
        db.pragma('foreign_keys = OFF');
        db.prepare('DELETE FROM sqlite_sequence WHERE name IN ("sales", "sale_items", "debt_ledger")').run();
        db.pragma('foreign_keys = ON');
    ...
    const saleItemsAfter = db.prepare('SELECT COUNT(*) as count FROM sale_items').get();
    const debtAfter = db.prepare('SELECT COUNT(*) as count FROM debt_ledger').get();

    console.log('✅ Verification:\n');
    console.log(`  Sales records:      ${salesAfter.count} (was ${salesBefore.count})`);
    console.log(`  Sale items:         ${saleItemsAfter.count} (was ${saleItemsBefore.count})`);
    console.log(`  Debt ledger:        ${debtAfter.count} (was ${debtBefore.count})`);
    console.log('');
