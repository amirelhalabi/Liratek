...
function getDatabasePath() {
  ...
  if (platform === 'darwin') {
    dbPath = path.join(os.homedir(), 'Library', 'Application Support', '@liratek', 'electron-app', 'liratek.db');
  } else if (platform === 'win32') {
    dbPath = path.join(os.homedir(), 'AppData', 'Roaming', '@liratek', 'electron-app', 'liratek.db');
  } else {
    dbPath = path.join(os.homedir(), '.config', '@liratek', 'electron-app', 'liratek.db');
  }
  return dbPath;
}
