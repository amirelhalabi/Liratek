...
function readParenGroup(s, i) {
  ...
  for (let j = i; j < s.length; j++) {
    ...
    if (c === "(") depth++;
      ...
      depth--;
      if (depth === 0) return { body: body.slice(1, -1), end: j };
    }
  }
...
for (const file of scannedFiles) {
  ...
  while ((idx = src.indexOf(".prepare(", idx)) !== -1) {
    ...
    if (src[p] !== ".") {
      idx = btEnd;
      continue;
    } // not an inline chain (prepared-const reuse)
    p++;
    ...
    if (args.some((a) => a.startsWith("..."))) {
      unhandled.push(`${rel}:${line} (spread args)`);
      idx = btEnd;
      continue;
    }
...
if (findings.length === 0) {
  console.log(
    "OK: no inline .prepare().run/get/all() placeholder/arg mismatches.",
  );
} else {
  console.log(`FOUND ${findings.length} placeholder/arg mismatch(es):\n`);
  for (const f of findings)
    console.log(
      `${f.rel}:${f.line}  .${f.method}()  ${f.ph} placeholders vs ${f.args} args\n    ${f.sql}\n`,
    );
}
if (unhandled.length)
  console.log(
