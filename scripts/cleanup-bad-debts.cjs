...
function getDatabasePath() {
  ...
  if (platform === 'darwin') {
    ...
    dbPath = path.join(os.homedir(), 'Library', 'Application Support', 'liratek', 'phone_shop.db');
  } else if (platform === 'win32') {
    // Windows
    dbPath = path.join(os.homedir(), 'AppData', 'Roaming', 'liratek', 'phone_shop.db');
  } else {
    // Linux
...
try {
  ...
  clients.forEach(client => {
    console.log(`  - ${client.full_name} (ID: ${client.id}, Phone: ${client.phone_number})`);
  });
  console.log('');
  ...
  debts.forEach(debt => {
    ...
    console.log(`  - ID: ${debt.id}, Client: ${client.full_name}, Type: ${debt.transaction_type}, Amount USD: ${debt.amount_usd}, Sale ID: ${debt.sale_id}`);
  });
  console.log('');
  ...
  if (saleIds.length > 0) {
    ...
    sales.forEach(sale => {
      ...
      console.log(`  - Sale ID: ${sale.id}, Client: ${client?.full_name || 'N/A'}, Amount: $${sale.final_amount_usd}, Status: ${sale.status}`);
    });
    console.log('');
  }
  ...
  readline.question('', (answer) => {
    if (answer.toLowerCase() === 'yes' || answer.toLowerCase() === 'y') {
      try {
        ...
        console.log('');
        console.log('ℹ️  Note: The client records were NOT deleted, only their debts and test sales.');
