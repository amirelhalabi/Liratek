...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    description:
      "Add BILL to financial_services.service_type CHECK constraint for iPick/Katsh bill processing",
    type: "typescript",
    up(db) {
      db.exec(`
        CREATE TABLE financial_services_v105 (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          provider TEXT CHECK(provider IN ('OMT', 'WHISH', 'BOB', 'OTHER', 'iPick', 'Katsh', 'WISH_APP', 'OMT_APP', 'BINANCE')) NOT NULL,
      ...
      db.exec(`
        INSERT INTO financial_services_v105
        SELECT
      ...
      db.exec(`
        DROP TABLE financial_services;
        ALTER TABLE financial_services_v105 RENAME TO financial_services;
      `);
      ...
      console.log(
        "Migration v105: Added BILL to financial_services.service_type CHECK",
      );
    },
    down(db) {
      db.exec(`DELETE FROM financial_services WHERE service_type = 'BILL';`);
      console.log(
        "Migration v105 rolled back (BILL rows removed; CHECK not downgraded — SQLite limitation)",
      );
    },
  },
];
