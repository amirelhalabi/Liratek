...
function createTestDb(): Database.Database {
  ...
  db.exec(`
    ...
    -- v165 (ProductRepository.COST_TIERS_SUBQUERY): getProducts() ->
    -- findAllProducts now always SELECTs a correlated subquery counting
    -- distinct open-batch unit costs from product_stock_batches. A
    -- correlated subquery's referenced table is resolved at PREPARE time
    -- regardless of row count, so a missing table throws even though this
    -- file never seeds a batch. No REFERENCES clause, matching the house
    -- convention (CustomServiceRepository.stock.test.ts).
