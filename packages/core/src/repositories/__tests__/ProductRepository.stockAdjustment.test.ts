...
function createTestDb(): Database.Database {
  ...
  db.exec(`
    ...
    CREATE TABLE stock_adjustments (
      ...
      -- v165: nullable, no backfill — StockAdjustmentRepository.create()'s
      -- INSERT column list references this column unconditionally
      -- (ProductRepository.adjustStock/adjustStockDelta always pass
      -- unit_cost_usd: null), so its absence fails db.prepare() for every
      -- adjustment in this suite.
