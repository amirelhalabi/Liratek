import { getClosingRepository, } from "../repositories/ClosingRepository.js";
export class ClosingService {
    ...
    setOpeningBalances(data) {
        return this.repo.setOpeningBalances(data.closing_date, data.amounts, data.user_id || 1);
    }
    /**
     * Create a daily closing record
     */
    createDailyClosing(data) {
        return this.repo.createDailyClosing(data.closing_date, data.amounts, data.system_expected_usd || 0, data.system_expected_lbp || 0, data.variance_notes, data.report_path);
    }
    /**
...
// Singleton instance
let closingServiceInstance = null;
export function getClosingService() {
    if (!closingServiceInstance) {
        closingServiceInstance = new ClosingService();
    }
    return closingServiceInstance;
}
export function resetClosingService() {
