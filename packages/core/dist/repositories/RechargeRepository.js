...
export class RechargeRepository extends BaseRepository {
    ...
    getVirtualStock() {
        const mtc = this.db
            .prepare("SELECT SUM(stock_quantity) as total FROM products WHERE item_type = 'Virtual_MTC'")
            .get();
        const alfa = this.db
            .prepare("SELECT SUM(stock_quantity) as total FROM products WHERE item_type = 'Virtual_Alfa'")
            .get();
        return {
            mtc: mtc?.total || 0,
            alfa: alfa?.total || 0,
    ...
    processRecharge(data) {
        ...
        `);
          ...
          `)
                ...
                const methodDrawerName = paidBy === "CASH"
                    ? "General"
                    : paidBy === "OMT"
                        ? "OMT"
                        : paidBy === "WHISH"
        ...
        `);
                ...
                insertPayment.run(saleId, data.provider === "MTC" ? "MTC" : "Alfa", providerDrawerName, stockDelta, "Telecom balance sent", createdBy);
        ...
        `)
            ...
            console.log(`[RECHARGE] ${data.provider} ${data.type}: ${data.amount} @ $${data.price} paid_by=${data.paid_by_method || "CASH"}`);
...
// Singleton Instance
// =============================================================================
let rechargeRepositoryInstance = null;
export function getRechargeRepository() {
...
export function resetRechargeRepository() {
