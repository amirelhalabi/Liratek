...
export class SalesRepository extends BaseRepository {
    ...
    processSale(sale) {
        ...
        try {
            ...
            WHERE id = ?
          `);
                    updateStmt.run(finalClientId, sale.total_amount, sale.discount, sale.final_amount, sale.payment_usd, sale.payment_lbp, sale.change_given_usd || 0, sale.change_given_lbp || 0, sale.exchange_rate, sale.drawer_name || "General_Drawer_B", status, sale.note || null, saleId);
          ...
          `);
                    const saleResult = saleStmt.run(finalClientId, sale.total_amount, sale.discount, sale.final_amount, sale.payment_usd, sale.payment_lbp, sale.change_given_usd || 0, sale.change_given_lbp || 0, sale.exchange_rate, sale.drawer_name || "General_Drawer_B", status, sale.note || null);
                ...
                const paymentLines = sale.payments?.length
                    ...
                    : [
                        ...(paymentUsd
                            ? [{ method: "CASH", currency_code: "USD", amount: paymentUsd }]
                            : []),
                        ...(paymentLbp
                            ? [{ method: "CASH", currency_code: "LBP", amount: paymentLbp }]
                            : []),
                    ];
                db.prepare(`DELETE FROM payments WHERE source_type = 'SALE' AND source_id = ?`).run(saleId);
        ...
        `);
                ...
                for (const p of paymentLines) {
                    ...
                    insertPayment.run(saleId, p.method, drawerName, p.currency_code, p.amount, note, createdBy);
                ...
                if (changeUsd) {
                    insertPayment.run(saleId, "CASH", "General", "USD", -changeUsd, "Change given", createdBy);
                    upsertBalanceDelta.run("General", "USD", -changeUsd);
                }
                if (changeLbp) {
                    insertPayment.run(saleId, "CASH", "General", "LBP", -changeLbp, "Change given", createdBy);
        ...
        `);
                for (const item of sale.items) {
                    itemStmt.run(saleId, item.product_id, item.quantity, item.price, item.product_id, item.imei || null);
          ...
          VALUES (?, ?, 'sales', ?, ?)
        `);
                logStmt.run(1, // Default user ID
                "SALE", saleId, JSON.stringify({
    ...
    createSale(data) {
      ...
      `);
            const result = stmt.run(data.client_id, data.total_amount, data.discount, data.final_amount, data.payment_usd, data.payment_lbp, data.change_given_usd, data.change_given_lbp, data.exchange_rate, data.drawer_name, data.status, data.note);
    ...
    updateSale(id, data) {
        ...
        WHERE id = ?
      `);
            const result = stmt.run(data.client_id, data.total_amount, data.discount, data.final_amount, data.payment_usd, data.payment_lbp, data.change_given_usd, data.change_given_lbp, data.exchange_rate, data.drawer_name, data.status, data.note, id);
    ...
    addSaleItem(saleId, item) {
      ...
      `, saleId, item.product_id, item.quantity, item.price, item.product_id, item.imei || null);
        }
        catch (error) {
    ...
    getDashboardStats() {
        try {
            // Total Sales Today
            const salesResult = this.queryOne(`
        SELECT 
      ...
      `);
            ...
            const repaymentResult = this.queryOne(`
        SELECT 
      ...
      `);
            // Orders Count Today
            const ordersResult = this.queryOne(`
        SELECT COUNT(*) as count 
        FROM ${this.tableName} 
        WHERE DATE(created_at, 'localtime') = DATE('now', 'localtime') AND status = 'completed'
      `);
            // Active Clients Count
            const clientsResult = this.queryOne("SELECT COUNT(*) as count FROM clients");
            // Low Stock Items Count
            const stockResult = this.queryOne(`
        SELECT COUNT(*) as count 
        FROM products 
...
// Singleton Instance
// =============================================================================
let salesRepositoryInstance = null;
...
export function resetSalesRepository() {
