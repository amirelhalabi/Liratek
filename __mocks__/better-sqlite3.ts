...
export const mockStatement = { // Export mockStatement too for more granular control
    run: jest.fn(function(this: any, ..._args: any[]) { // Changed args to _args
        if (this._sql.trim().toUpperCase().startsWith('INSERT')) {
            return { changes: 1, lastInsertRowid: 1 };
        }
        return { changes: 1 };
    }),
    all: jest.fn(function(this: any, ..._args: any[]) { // Changed args to _args
        if (this._sql.includes('FROM expenses')) {
            return [];
        }
        if (this._sql.includes('FROM sales') || this._sql.includes('FROM debt_ledger')) {
            return [{ total_usd_sales: 0, total_lbp_sales: 0, total_usd_repayments: 0, total_lbp_repayments: 0, total_expenses_usd: 0, total_lbp_expenses: 0, total_profit_usd: 0 }];
        }
        return [];
    }),
    get: jest.fn(function(this: any, ..._args: any[]) { // Changed args to _args
...
export const mockDatabase = { // Export mockDatabase
