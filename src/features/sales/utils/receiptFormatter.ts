...
export function formatReceipt58mm(data: ReceiptData): string {
  ...
  receipt +=
    padRight("", 30) +
    `≈ ${(data.total * data.exchange_rate).toLocaleString()} LBP\n`;
  receipt += "\n";
  ...
  if (data.payment_lbp > 0) {
    receipt +=
      padRight("Paid LBP:", 30) + `${data.payment_lbp.toLocaleString()}\n`;
    receipt +=
      padRight("(@ Rate):", 30) +
  ...
  if (data.change_lbp > 0) {
    receipt +=
      padRight("Change LBP:", 30) + `${data.change_lbp.toLocaleString()}\n`;
  }
