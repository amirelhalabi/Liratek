            ...
            <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-lg overflow-hidden shadow-2xl">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="flex items-center gap-3 py-2">
                        <div className={`w-10 h-6 rounded-full p-1 cursor-pointer transition-colors ${formData.whatsapp_opt_in ? 'bg-green-500' : 'bg-slate-600'}`}
                            onClick={() => setFormData(prev => ({ ...prev, whatsapp_opt_in: !prev.whatsapp_opt_in }))}
                        >
                            <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform ${formData.whatsapp_opt_in ? 'translate-x-4' : ''}`}></div>
                        </div>
                        <span className="text-slate-300 text-sm flex items-center gap-2">
                            <MessageCircle size={16} className={formData.whatsapp_opt_in ? 'text-green-400' : 'text-slate-500'} />
