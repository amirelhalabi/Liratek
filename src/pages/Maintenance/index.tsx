...
export default function Maintenance() {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'Delivered': return 'text-slate-400 bg-slate-400/10';
                            ...
                            {jobs.map((job) => (
                                <tr key={job.id} className="hover:bg-slate-700/20 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="text-sm font-medium text-white">{job.client_name || 'Unknown'}</div>
                                        {job.client_phone && <div className="text-xs text-slate-500">{job.client_phone}</div>}
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-sm font-bold text-white">{job.device_name}</div>
                                        <div className="text-xs text-slate-400 truncate max-w-[200px]">{job.issue_description}</div>
                                    </td>
                                    <td className="px-6 py-4">
                        ...
                        <div className="p-6 overflow-y-auto space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="col-span-2">
                                    <label className="block text-xs font-medium text-slate-400 mb-1 uppercase">Device Name / Model</label>
                                    <input
                                        type="text"
                                        value={deviceName}
                                ...
                                <div className="col-span-2">
                                    <label className="block text-xs font-medium text-slate-400 mb-1 uppercase">Issue Description</label>
                                    <textarea
                                        value={issue}
                            ...
                            <div className="grid grid-cols-2 gap-4 p-4 bg-slate-900/50 rounded-xl border border-slate-700/50">
                                <div>
                                    <label className="block text-xs font-medium text-slate-400 mb-1 uppercase">Repair Cost (Internal)</label>
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-emerald-400 mb-1 uppercase">Price to Client</label>
                                    <div className="relative">
