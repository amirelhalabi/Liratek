                        ...
                        {products.map((product) => (
                            ...
                            >
                                <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <div className="bg-violet-600 text-white text-xs px-2 py-1 rounded-full">Add</div>
                                </div>
                                ...
                                <h3 className="font-semibold text-slate-200 line-clamp-2 mb-1">{product.name}</h3>
                                <div className="text-xs text-slate-500 mb-2">{product.category}</div>

                                <div className="mt-auto flex items-center justify-between w-full">
                                    <span className="text-violet-400 font-bold">${product.retail_price.toFixed(2)}</span>
                                    <span className={`text-xs px-2 py-0.5 rounded-full ${product.stock_quantity > 0 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
