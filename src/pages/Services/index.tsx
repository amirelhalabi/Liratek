...
export default function Services() {
    ...
    const providerColors: Record<Provider, { bg: string; activeBg: string; text: string }> = {
        OMT: { bg: 'bg-[#ffde00]/10', activeBg: 'bg-[#ffde00]', text: 'text-[#ffde00]' },
        WHISH: { bg: 'bg-[#ff0a46]/10', activeBg: 'bg-[#ff0a46]', text: 'text-[#ff0a46]' }
    };
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-slate-800 p-5 rounded-xl border border-slate-700/50 shadow-lg hover:border-slate-600 transition-colors">
                    ...
                    <h3 className="text-slate-500 text-xs font-medium uppercase tracking-wider">Today's Earnings</h3>
                    <p className="text-2xl font-bold text-white mt-1">${analytics.today.commissionUSD.toFixed(2)}</p>
                    {analytics.today.commissionLBP > 0 && (
                        <p className="text-xs text-slate-400 mt-1">+ {analytics.today.commissionLBP.toLocaleString()} LBP</p>
                    )}
                    <p className="text-xs text-slate-500 mt-1">{analytics.today.count} transactions</p>
                </div>
                ...
                <div className="bg-slate-800 p-5 rounded-xl border border-slate-700/50 shadow-lg hover:border-slate-600 transition-colors">
                    ...
                    <h3 className="text-slate-500 text-xs font-medium uppercase tracking-wider">Monthly Earnings</h3>
                    <p className="text-2xl font-bold text-white mt-1">${analytics.month.commissionUSD.toFixed(2)}</p>
                    {analytics.month.commissionLBP > 0 && (
                        <p className="text-xs text-slate-400 mt-1">+ {analytics.month.commissionLBP.toLocaleString()} LBP</p>
                    )}
                    <p className="text-xs text-slate-500 mt-1">{analytics.month.count} transactions</p>
                </div>

                {/* Provider breakdown - Show up to 2 */}
                {analytics.byProvider.slice(0, 2).map(p => (
                    <div key={p.provider} className="bg-slate-800 p-5 rounded-xl border border-slate-700/50 shadow-lg hover:border-slate-600 transition-colors">
                        <div className="flex items-center justify-between mb-3">
                            <div className={`p-2 rounded-lg ${providerColors[p.provider as Provider].bg}`}>
                                <Send className={`w-5 h-5 ${providerColors[p.provider as Provider].text}`} />
                            </div>
                        </div>
                        <h3 className="text-slate-500 text-xs font-medium uppercase tracking-wider">{p.provider} Today</h3>
                        <p className="text-2xl font-bold text-white mt-1">${p.commission_usd.toFixed(2)}</p>
                        <p className="text-xs text-slate-500 mt-1">{p.count} transactions</p>
                    </div>
                ))}
            </div>
            ...
            <div className="flex gap-6 h-[calc(100vh-theme(spacing.64))]">
                ...
                <div className="w-1/3 min-w-[380px] bg-slate-800 rounded-xl border border-slate-700/50 shadow-lg p-6 flex flex-col">
                    ...
                    <div className="flex gap-2 mb-6">
                        {(['SEND', 'RECEIVE', 'BILL_PAYMENT'] as ServiceType[]).map(type => {
                            ...
                            return (
                                ...
                                >
                                    <Icon size={18} />
                                    {type === 'BILL_PAYMENT' ? 'Bill' : type.charAt(0) + type.slice(1).toLowerCase()}
                                </button>
                            );
                        })}
                    </div>
                    ...
                    <div className="space-y-4 flex-1">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider">Amount USD</label>
                                <div className="relative">
                                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#ffde00] font-bold">$</span>
                                    <input
                                        type="number"
                                        value={amountUSD}
                            ...
                            <div>
                                <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider">Amount LBP</label>
                                <input
                                    type="number"
                                    value={amountLBP}
                        ...
                        <div className="p-4 rounded-xl bg-[#ffde00]/5 border border-[#ffde00]/20">
                            <label className="block text-xs font-medium text-[#ffde00] mb-3 uppercase tracking-wider">Commission / Profit</label>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="relative">
                                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#ffde00] font-bold">$</span>
                                    <input
                                    ...
                                    return (
                                        <tr key={tx.id} className="hover:bg-slate-700/20 transition-colors">
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${providerBadgeColors[tx.provider]}`}>
                                                    {tx.provider}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2 text-slate-300">
                                                    ...
                                                    <span className="text-sm">
                                                        {tx.service_type === 'BILL_PAYMENT' ? 'Bill' : tx.service_type.charAt(0) + tx.service_type.slice(1).toLowerCase()}
                                                    </span>
                                                </div>
                                            </td>
