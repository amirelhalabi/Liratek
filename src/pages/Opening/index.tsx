...
export default function Opening({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  const drawerTypes: Array<'General' | 'OMT' | 'MTC' | 'Alfa'> = ['General','OMT','MTC','Alfa'];
  const { user } = useAuth();
  const [currencies, setCurrencies] = useState<Array<{ code: string; name: string }>>([]);
  const [amountsText, setAmountsText] = useState<Record<string, Record<string, string>>>({}); // drawer -> currency -> opening (text)
  const [drawerType, setDrawerType] = useState<'General' | 'OMT' | 'MTC' | 'Alfa'>('General');
  ...
  const setDrawerCurrencyText = (drawer: string, code: string, value: string) => {
    setAmountsText(prev => ({ ...prev, [drawer]: { ...prev[drawer], [code]: value } }));
  };

  const handleSave = async () => {
    try {
      const closing_date = new Date().toISOString().split('T')[0];
      const amountsArray = [] as Array<{ drawer_name: string; currency_code: string; opening_amount: number }>;
      for (const d of drawerTypes) {
        for (const c of currencies) {
          const raw = amountsText[d]?.[c.code] ?? '';
          const parsed = raw === '' ? 0 : parseFloat(raw);
          amountsArray.push({ drawer_name: d, currency_code: c.code, opening_amount: isNaN(parsed) ? 0 : parsed });
        }
      }
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 shadow-2xl w-full max-w-xl overflow-hidden">
        ...
        <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {currencies.map(c => (
                    <div key={c.code}>
                      ...
                      <input type="number" value={amountsText[d]?.[c.code] ?? ''} onChange={(e)=> setDrawerCurrencyText(d, c.code, e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white" />
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="flex justify-end gap-3 mt-4">
            <button onClick={onClose} className="px-4 py-2 rounded-lg bg-slate-700 text-slate-300">Cancel</button>
            <button onClick={handleSave} className="px-4 py-2 rounded-lg bg-emerald-600 text-white">Save Opening</button>
          </div>
        </div>
      </div>
    </div>
