...
import { Smartphone, Signal, Wifi, CreditCard, Hash, DollarSign, CheckCircle } from 'lucide-react';
...
export default function Recharge() {
    ...
    const [rechargeType, setRechargeType] = useState<RechargeType>('CREDIT_TRANSFER');
    ...
    // Theme Colors
    const theme = activeProvider === 'MTC' 
        ? { 
            primary: 'bg-cyan-600', 
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-4">
                    <div className="bg-slate-800 px-4 py-2 rounded-lg border border-cyan-500/30 flex items-center gap-3">
                        ...
                        <span className="text-slate-400 text-xs uppercase font-bold">MTC Stock</span>
                        <span className="text-cyan-400 font-mono font-bold">${stock.mtc.toFixed(2)}</span>
                    </div>
                    <div className="bg-slate-800 px-4 py-2 rounded-lg border border-red-500/30 flex items-center gap-3">
                        <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                        <span className="text-slate-400 text-xs uppercase font-bold">Alfa Stock</span>
                        <span className="text-red-400 font-mono font-bold">${stock.alfa.toFixed(2)}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="flex gap-6 h-[calc(100vh-theme(spacing.48))]">
                ...
                <div className="w-1/4 min-w-[250px] flex flex-col gap-4">
                    ...
                    <div className="bg-slate-800 p-4 rounded-xl border border-slate-700/50 flex-1 flex flex-col gap-2">
                        <label className="text-xs font-medium text-slate-500 uppercase mb-2">Service Type</label>
                        {[
                            { id: 'CREDIT_TRANSFER', label: 'Credit Transfer', icon: DollarSign },
                    ...
                    <div className="max-w-md mx-auto w-full space-y-8">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                <div className="relative">
                                    <span className={`absolute left-4 top-1/2 -translate-y-1/2 font-bold ${theme.text}`}>$</span>
                                    <input
                                        type="number"
                                        value={amount}
                        ...
                        <button
                            ...
                            className={`w-full py-5 rounded-xl font-bold text-xl text-white shadow-lg active:scale-95 transition-all flex items-center justify-center gap-3 ${
                                isSubmitting ? 'bg-slate-600 cursor-not-allowed' : `${theme.primary} ${theme.hover} ${theme.shadow}`
                            }`}
                        >
