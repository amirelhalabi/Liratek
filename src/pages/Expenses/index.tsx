...
const EXPENSE_CATEGORIES = ['Shop_Supply', 'Bill', 'Inventory_Loss', 'Refund_Damaged', 'Other'];
const EXPENSE_TYPES = [
...
export default function Expenses() {
    ...
    const handleAddExpense = async () => {
        if (!formData.description.trim() || (formData.amount_usd === 0 && formData.amount_lbp === 0)) {
            ...
            <div className="max-w-6xl mx-auto space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                    <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 shadow-xl">
                        ...
                        <p className="text-3xl font-bold text-orange-400">${totalUSD.toFixed(2)}</p>
                    </div>
                    <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 shadow-xl">
                        <p className="text-slate-400 text-sm font-medium mb-2">Total LBP</p>
                        <p className="text-3xl font-bold text-orange-400">{totalLBP.toLocaleString()}</p>
                    </div>
                    <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 shadow-xl">
                        <p className="text-slate-400 text-sm font-medium mb-2">Cash Out (Affects Drawer)</p>
                        <p className="text-3xl font-bold text-red-400">${cashOutUSD.toFixed(2)}</p>
                    </div>
                </div>
                                ...
                                <tbody className="divide-y divide-slate-700">
                                    {expenses.map(expense => (
                                        <tr key={expense.id} className="hover:bg-slate-700/30 transition-colors">
                                            <td className="px-6 py-4 text-slate-200">{expense.description}</td>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={e => setFormData({ ...formData, expense_type: e.target.value as 'Cash_Out' | 'Non_Cash' })}
                                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-orange-500 outline-none"
                                >
                            ...
                            {/* Amount LBP */}
                            <div>
                                <label className="block text-sm font-medium text-slate-300 mb-2">Amount (LBP)</label>
                                <input
                                    type="number"
                                    value={formData.amount_lbp || ''}
                                    onChange={e => setFormData({ ...formData, amount_lbp: parseFloat(e.target.value) || 0 })}
                                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-orange-500 outline-none"
                                    placeholder="0"
                                />
                            </div>
