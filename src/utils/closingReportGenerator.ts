export function generateClosingReport(closingData: any, dailyStats: any): string {
    const usdVariance = closingData.physical_usd - closingData.system_expected_usd;
    const lbpVariance = closingData.physical_lbp - closingData.system_expected_lbp;
    const eurVariance = closingData.physical_eur - (closingData.system_expected_eur || 0); // Ensure EUR variance is calculated correctly

    const formatVariance = (variance: number, systemExpected: number, currency: string) => {
        ...
        if (variance > 0) {
            ...
            varianceSign = '-';
            status = 'Deficit';
        } else { // variance === 0, already handled above
