...
export function formatReceipt58mm(data: ReceiptData): string {
    ...
    receipt += padRight('Item', 20) + padRight('Qty', 6) + 'Price\n';
    receipt += '-'.repeat(width) + '\n';
    ...
    // Totals
    receipt += padRight('Subtotal:', 30) + `$${data.subtotal.toFixed(2)}\n`;
    if (data.discount > 0) {
        receipt += padRight('Discount:', 30) + `-$${data.discount.toFixed(2)}\n`;
    }
    receipt += padRight('TOTAL:', 30) + `$${data.total.toFixed(2)}\n`;
    receipt += padRight('', 30) + `≈ ${(data.total * data.exchange_rate).toLocaleString()} LBP\n`;
    receipt += '\n';

    // Payment
    receipt += 'PAYMENT\n';
    receipt += '-'.repeat(width) + '\n';
    receipt += padRight('Paid USD:', 30) + `$${data.payment_usd.toFixed(2)}\n`;
    if (data.payment_lbp > 0) {
        receipt += padRight('Paid LBP:', 30) + `${data.payment_lbp.toLocaleString()}\n`;
        receipt += padRight('(@ Rate):', 30) + `$${(data.payment_lbp / data.exchange_rate).toFixed(2)}\n`;
    }
    receipt += padRight('Change USD:', 30) + `$${data.change_usd.toFixed(2)}\n`;
    if (data.change_lbp > 0) {
        receipt += padRight('Change LBP:', 30) + `${data.change_lbp.toLocaleString()}\n`;
    }
...
export function formatReceipt80mm(data: ReceiptData): string {
    ...
    receipt += `Date: ${new Date(data.timestamp).toLocaleDateString()}  Time: ${new Date(data.timestamp).toLocaleTimeString()}`.padEnd(width) + '\n';
    receipt += '\n';
    ...
    receipt += padRight('(LBP Equivalent):', 46) + `${(data.total * data.exchange_rate).toLocaleString()}\n`;
    receipt += '\n';

    // Payment Info
    receipt += 'PAYMENT METHOD\n';
    receipt += '─'.repeat(width) + '\n';
    receipt += padRight('USD Received:', 46) + `$${data.payment_usd.toFixed(2)}\n`;
    if (data.payment_lbp > 0) {
        receipt += padRight('LBP Received:', 46) + `${data.payment_lbp.toLocaleString()}\n`;
        receipt += padRight(`(@ ${data.exchange_rate}/USD)`, 46) + `$${(data.payment_lbp / data.exchange_rate).toFixed(2)}\n`;
    }
    receipt += padRight('Change - USD:', 46) + `$${data.change_usd.toFixed(2)}\n`;
    if (data.change_lbp > 0) {
        receipt += padRight('Change - LBP:', 46) + `${data.change_lbp.toLocaleString()}\n`;
    }
