...
const NotificationCenter: React.FC = () => {
    ...
    useEffect(() => {
        const handleShowNotification = (message: string, type: 'success' | 'error' | 'info' | 'warning', duration?: number) => {
            const id = Date.now().toString();
            const newNotification: Notification = { id, message, type, duration };
            setNotifications(prev => [...prev, newNotification]);
        };

        const unsubscribe = appEvents.on('notification:show', handleShowNotification);
    ...
    const getBgColorClass = (type: Notification['type']) => {
        switch (type) {
            case 'success': return 'bg-emerald-900/80 border-emerald-500';
            case 'error': return 'bg-red-900/80 border-red-500';
            case 'info': return 'bg-blue-900/80 border-blue-500';
            case 'warning': return 'bg-amber-900/80 border-amber-500';
            default: return 'bg-slate-800/80 border-slate-600';
        }
    };
        ...
        <div className="fixed bottom-4 right-4 z-[1000] space-y-3">
            {notifications.map(notification => (
                ...
                >
                    ...
                    <button onClick={() => setNotifications(prev => prev.filter(n => n.id !== notification.id))} className="text-white/70 hover:text-white transition-colors">
                        <XCircle size={16} />
                    </button>
                </div>
            ))}
