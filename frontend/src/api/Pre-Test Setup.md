...
-- Check transactions
SELECT _ FROM transactions WHERE source_table IN ('loto_tickets', 'loto_cash_prizes', 'loto_checkpoints');
-- Check supplier ledger
SELECT _ FROM supplier_ledger WHERE supplier_id = (SELECT id FROM suppliers WHERE provider = 'LOTO');
